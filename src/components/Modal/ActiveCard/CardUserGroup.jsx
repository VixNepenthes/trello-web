...
function CardUserGroup({ cardMemberIds = [], onUpdateCardMembers }) {
  ...
  const cardMembers = cardMemberIds.map((memberId) => board.allUsersInBoard.find((user) => user._id === memberId))

  function handleUpdateCardMembers(user) {
  ...
  return (
    <Box sx={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
      ...
      <Popover id={popoverId} open={isOpenPopover} anchorEl={anchorPopoverElement} onClose={handleTogglePopover} anchorOrigin={{ vertical: 'bottom', horizontal: 'left' }}>
        <Box sx={{ p: 2, maxWidth: '260px', display: 'flex', flexWrap: 'wrap', gap: 1.5 }}>
          {board?.allUsersInBoard?.map((user, index) => (
            <Tooltip title={user.displayName} key={index}>
              <Badge
                sx={{ cursor: 'pointer' }}
                overlap="rectangular"
                anchorOrigin={{ vertical: 'bottom', horizontal: 'right' }}
                badgeContent={cardMemberIds.includes(user._id) && <CheckCircleIcon fontSize="small" sx={{ color: '#27ae60' }} />}
